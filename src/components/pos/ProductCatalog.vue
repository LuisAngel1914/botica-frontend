<template>
  <section class="app-card flex min-h-0 flex-col overflow-hidden">
    ...
    <div v-if="!query && quickProducts.length" class="border-b border-slate-100 bg-slate-50/70 p-4">
      <div class="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-500"><Heart :size="14" class="text-rose-500" />Acceso rápido</div>
      <div class="flex gap-2 overflow-x-auto pb-1">
        <div v-for="product in quickProducts" :key="product.id" class="flex shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs">
          <button class="max-w-36 px-3 py-2 text-left text-xs font-semibold text-slate-700 hover:bg-cyan-50" @click="$emit('select', product)"><span class="block truncate">{{ product.nombre }}</span><span class="mt-0.5 block text-cyan-700">S/ {{ money(product.precio_venta) }}</span></button>
          <button class="border-l border-slate-100 px-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600" :aria-label="'Alternar favorito: ' + product.nombre" @click="$emit('toggle-favorite', product)"><Heart :size="15" :fill="favoriteIds.includes(product.id) ? 'currentColor' : 'none'" :class="favoriteIds.includes(product.id) ? 'text-rose-500' : ''" /></button>
        </div>
      </div>
    </div>
